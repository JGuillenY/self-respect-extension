...
export function isDomainBlocked(domain) {
    ...
    return DOMAIN_CATEGORIES.some(category => category.domains.some(d => d.toLowerCase() === normalizedDomain));
}
// Helper function to get redirect URL for a domain
export function getRedirectUrlForDomain(domain) {
    const normalizedDomain = domain.toLowerCase().trim();
    const category = DOMAIN_CATEGORIES.find(category => category.domains.some(d => d.toLowerCase() === normalizedDomain));
    return category?.redirectTo || DEFAULT_REDIRECT_URL;
}
// Export all domains as a flat array for quick lookups
export const ALL_BLOCKED_DOMAINS = DOMAIN_CATEGORIES.flatMap(category => category.domains);
// Original arrays for backward compatibility
export const pornDomains = DOMAIN_CATEGORIES.find(c => c.name === "Adult Content")?.domains || [];
export const rssDomains = RSS_DOMAINS.flatMap(category => category.domains);
