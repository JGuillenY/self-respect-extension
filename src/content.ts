    ...
    <div class="sudoku-container">
      <div class="sudoku-header">
        ...
        <p>
          ...
          To access this site, you must solve a hard Sudoku puzzle.
      ...
      <div class="sudoku-game-area">
        ...
        <div class="sudoku-instructions">
          ...
          <ul>
            ...
            <li>Use <strong>Check Solution</strong> to validate your progress</li>
        ...
        <div class="sudoku-success" id="sudoku-success">
          <h3>🎉 Puzzle Solved!</h3>
          <p>You successfully completed the Sudoku puzzle in <span id="sudoku-final-time">00:00</span>.</p>
          <div class="sudoku-action-buttons">
        ...
        <div class="sudoku-error" id="sudoku-error">
...
function placeSudokuNumber(num: number): void {
  ...
  if (checkSudokuWinCondition()) {
    ...
    const finalTimeElement = document.getElementById('sudoku-final-time');
    if (successElement && finalTimeElement) {
      const elapsedSeconds = Math.floor((Date.now() - sudokuGameState.startTime) / 1000);
...
function initializeSudokuGame(): void {
  ...
  // Remove cells for hard difficulty (55 cells removed = 26 clues)
...
function checkSudokuSolution(): void {
  ...
  for (let row = 0; row < 9; row++) {
    for (let col = 0; col < 9; col++) {
      ...
      if (value === 0) {
        emptyCells++;
      } else if (value !== sudokuGameState.solution[row][col]) {
        hasErrors = true;
      }
    }
  }
  ...
  if (hasErrors) {
    errorElement.textContent = 'There are incorrect numbers in the puzzle. Keep trying!';
    errorElement.classList.add('show');
    setTimeout(() => errorElement.classList.remove('show'), 3000);
  } else if (emptyCells > 0) {
    errorElement.textContent = `All filled numbers are correct! ${emptyCells} cells remaining.`;
    errorElement.classList.add('show');
    setTimeout(() => errorElement.classList.remove('show'), 3000);
  } else {
    // This shouldn't happen since win condition should catch it
    sudokuGameState.isSolved = true;
    stopTimer();
